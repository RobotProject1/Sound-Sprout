...
import time
from threading import Thread
from plant_classification import read_id_stable
...
class ReadNWrite(Thread):
    ...
    def run(self):
        while True:
            plant_id_new = read_id_stable()  # This now returns stable IDs
            if plant_id_new != self.plant_id_old:
                path_list = ",".join(track[i] for i in plant_id_new if i != 0)
                with open('sound_sprout/path_list.txt', 'w') as file:
                    file.write(path_list)
                self.plant_id_old = plant_id_new.copy()
